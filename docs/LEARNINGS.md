...
## 2026-07-29 - "CI passed" is not the same claim as "the real download works everywhere"

- **Concept: a build and its own test can share the same blind spot.**
  `release.yml`'s Linux job builds the binary on `ubuntu-latest` and then smoke-tests it on the same `ubuntu-latest` runner - so the test can never catch a compatibility problem that's specific to the build environment itself, only problems specific to the *code*. Downloading the actual published `v0.1.0` Linux asset and running it for real inside `debian:bookworm-slim` and `ubuntu:22.04` containers (genuine Linux environments, via Docker Desktop's real Linux VM, not emulation) found exactly that kind of problem: `GLIBC_2.38' not found` - the binary was built against a newer glibc than Debian 12 or Ubuntu 22.04 LTS actually ship, so it would fail to even start on a large slice of currently-deployed Linux servers, despite CI being green the whole time.
  How to explain it to someone else: "a CI check that builds and tests on the identical machine image can only prove the code works there - it can't prove the artifact works anywhere else, and 'anywhere else' is the entire point of shipping a portable binary."
- **Concept: a compiled binary's compatibility floor is set by the oldest glibc it was built against, not the newest.**
  Rebuilding inside `python:3.11-slim-bookworm` (Debian 12's own glibc, 2.36) instead of directly on the `ubuntu-latest` runner (glibc 2.39-ish) produced a binary that ran correctly on Debian 12, Ubuntu 22.04, and Ubuntu 24.04 alike - glibc compatibility is one-directional (older-built binaries run fine on newer systems, never the reverse), so building against the *oldest* target you still care about is strictly better than building against whatever happens to be newest.
  How to explain it to someone else: "when you build a portable binary, the question isn't 'what's on my build machine' - it's 'what's the oldest real system this needs to run on,' and you build against that one on purpose."
- **Process: the Windows leg got a different kind of proof - reading the real CI log, not re-running it.**
  For Windows, there's no local Windows machine to test against, so instead of re-running anything, the actual `windows-latest` GitHub Actions job log from the `v0.1.0` release run was pulled directly and read: it shows the genuine published `.exe`, executed on a real Windows Server runner, producing the exact expected report and exit codes. That's real evidence from a real Windows environment - just already-captured rather than freshly reproduced.
  How to explain it to someone else: "verifying something doesn't always mean re-running it yourself - sometimes the proof already exists in a log, and reading it carefully is the verification."
