...
PRD Section 4.2 lists SEPs (SEP-2575, the SEP-2663/SEP-2557 tasks discrepancy, SEP-2322) that describe likely-real changes, but that aren't safe to build rules against yet — either because the underlying proposal hasn't merged, or because which exact SEP number is the right citation is still unclear.

Once SEP-2575 (and related work) actually merges with a stable, final shape, its mechanisms — `server/discover`, `subscriptions/listen`, the removal of `ping`/`logging/setLevel`/`notifications/roots/list_changed` — become real candidates for new Confirmed rules, the same way R1-R3 are today. The same applies to SEP-2322's `resultType` field once its final shape is confirmed as unchanged from what's in the draft today.
